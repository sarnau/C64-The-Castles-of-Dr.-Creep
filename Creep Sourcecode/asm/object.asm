...
; 
; $0800 - $77ff:  Loaded Game Code - Area from 'StoreRoomData' on will be overwritten by a copy of the actual room
; 
...
.ChkBlinkColor      lda CCL_Player1TargetRoomNo     ; .hbu016.
                    cmp CCL_Player2TargetRoomNo     ; .hbu016.
                    bne .GetBlinkColorDiff          ; .hbu016. - enter same room
                    
                    lda CCL_Player1TargetDoorNo     ; .hbu016.
                    cmp CCL_Player2TargetDoorNo     ; .hbu016.
                    bne .GetBlinkColorDiff          ; .hbu016. - enter same room via different doors
                    
.GetBlinkColorSame  lda #CC_MultiColor0Player1      ; .hbu016. - init color for same room/door
                    jmp .IniBlinkColor              ; .hbu016.
                    
.GetBlinkColorDiff  lda #BLACK                      ; .hbu016. - let player1 blink if player2 is off and vice versa
.IniBlinkColor      sta SP0COL                      ; .hbu016. - VIC 2 - $D027 = Color Sprite 0
                    lda #CC_MultiColor0Player2      ; .hbu016.
                    sta SP1COL                      ; .hbu016. - VIC 2 - $D028 = Color Sprite 1
...
.BlinkOne           lda TabColorPlayer,x            ; .hbu016. - init colors after call copy CopySpriteData
                    eor SP0COL                      ; .hbu016. - VIC 2 - $D027 = Color Sprite 0
                    sta SP0COL                      ; .hbu016. - VIC 2 - $D027 = Color Sprite 0
                    
                    lda #$01                        ; .hbu016. - enable sprite 1
                    jmp .BlinkSpriteEnabSet         ; .hbu016.
                    
.BlinkBoth          lda CCL_Player1TargetDoorNo     ; .hbu016.
                    cmp CCL_Player2TargetDoorNo     ; .hbu016.
                    bne .BlinkBothDiff              ; .hbu016. - enter same room via different doors
                    
.BlinkBothSame      lda CCZ_SpritesEnab             ; .hbu016. - enter same room via same door - flip 2 colors (no black)
                    cmp #$01                        ; .hbu016.
                    beq .BlinkBothSameGet2          ; .hbu016.
                    
.BlinkBothSameGet1  lda #$01                        ; .hbu016. - enable sprite 1
                    bne .BlinkSpriteGetPort         ; .hbu016. - always check port 0
                    
.BlinkBothSameGet2  lda #$02                        ; .hbu016. - enable sprite 2
                    bne .BlinkSpriteGetPort         ; .hbu016. - always check port 0
                    
.BlinkBothDiff      lda #CC_MultiColor0Player1      ; .hbu016. - flip color 1 and black
                    eor SP0COL                      ; .hbu016. - VIC 2 - $D027 = Color Sprite 0
                    sta SP0COL                      ; .hbu016. - VIC 2 - $D027 = Color Sprite 0
                    lda #CC_MultiColor0Player2      ; .hbu016. - flip color 2 and black
                    eor SP1COL                      ; .hbu016. - VIC 2 - $D028 = Color Sprite 1
                    sta SP1COL                      ; .hbu016. - VIC 2 - $D028 = Color Sprite 1
                    
.BlinkBothSameGet12 lda #$03                        ; .hbu016. - enable sprite 1 & 2
                    
.BlinkSpriteGetPort ldx #$00                        ; set player1 check fire (ignoring player2)
                    
.BlinkSpriteEnabSet sta CCZ_SpritesEnab             ; .hbu016. - enable sprite(s)
...
;.ChkKey_Stop        lda CCW_KeyGotStop              ; .hbu020.
;                    cmp #CCW_KeyGotYes              ; 
;                    bne .ChkJoy_Fire                ; check if a guy pressed the fire button
;                    
...
.LivesOnOff         lda CCW_LoadCtrlUnlimLives      ; 
                    ...
                    clc                             ; 
                    adc #ScreenLineLivesOn - ScreenLineLivesTxt ; offset 'on'
                    tay                             ; 
                    ldx #$00                        ; 
...
;.ChkResume          cmp #CC_LoadCtrlIdResume        ; .hbu020.
;                    bne .ChkTimes                   ; 
...
.ChkTimes           cmp #CC_LoadCtrlIdTimes         ; 
                    bne .Exit                       ; CC_LoadCtrlIdExit not checked yet and left over - .hbu020.
                    
.ShowTimes          lda CCW_LoadCtrlTabOff          ; 
                    cmp #$ff                        ; 
...
.ChkDemo            lda CCW_DemoFlag                ; 
                    ...
                    ldy #CC_Obj_GunPoleLen          ; .hbu021.
                    lda (CCZ_RoomGunMod),y          ; .hbu021.
                    bne .ChkGunMoves                ; .hbu021.
                    
                    jmp .Stop                       ; .hbu021.
                    
.ChkGunMoves        ldy #CC_Obj_GunDirection        ; 
...
.ChkMaxGuns         lda CCW_GunDataNext             ; max six guns allowed
                    cmp #$05                        ; 
                    bcs AutoRayGunAimX              ; 
...
;                   Returns : 
; ------------------------------------------------------------------------------------------------------------- ;
;GetDiskFileName     subroutine                      ; .hbu020.
;                    jsr InitHiResSpriteWAs          ; initialize the hires screen and sprite work area
;                    
;                    lda #<TextFileName              ; 
;                    sta CCZ_RoomItemLo              ; 
...
.InitDeath          eor #CC_WaS_FlagDead            ; reset death flag
                    ...
                    pha                             ; .hbu017.
.GetSavedRoomData   jsr RoomDataRestore             ; .hbu017. - restore saved room data after a players death
                    pla                             ; .hbu017.
...
RunIntoRoomX        jmp RoomDataSave                ; .hbu017. - Save player status / original room data for a restore after death
; ------------------------------------------------------------------------------------------------------------- ;
...
RoomDataSaveX       rts
; ------------------------------------------------------------------------------------------------------------- ;
; RoomDataRestore   Function: Restore the saved room data after a players death
;                   Parms   : 
;                   Returns : 
;                   Id      : .hbu017.
; ------------------------------------------------------------------------------------------------------------- ;
...
.LoadFirstCastle    ldx #CC_LoadCtrlNoFixItems * CC_LoadCtrlEntryLen ; load screen ctrl tab offset 1st castle data file name
...
.ScreenLineOutInit  ldy #ScreenLineRow              ; screen row number
                    ...
                    ldy #ScreenLineCol              ; screen column number
...
.ReadDirectory      lda #CCW_DiskFileIdDir          ; $0:Z* - all castle data files names start with 'z'
                    ...
                    lda #CC_LoadCtrlColAreaLe       ; default start column of left output area
...
.ChkLeftArea        lda CCW_LoadCtrlDynAreaCol      ; 
                    cmp #CC_LoadCtrlColAreaLe       ; default start column of left  output area
                    bne .SetLeftArea                ; 
                    
.SetRightArea       lda #CC_LoadCtrlColAreaRi       ; default start column of right output area
                    sta CCW_LoadCtrlDynAreaCol      ; 
.GoGetNextChr       jmp .SetNextChr                 ; 
                    
.SetLeftArea        lda #CC_LoadCtrlColAreaLe       ; 
                    sta CCW_LoadCtrlDynAreaCol      ; 
...
.Exit               ldx CCW_LoadCtrlTabOffWrk       ; actual screen control table offset
                    ...
                    lda #CC_LoadCtrlPosInit * CC_LoadCtrlEntryLen ; init pointer to unlim lives on/off entry
                    sta CCW_LoadCtrlTabOffWrk       ; actual screen control table offset
