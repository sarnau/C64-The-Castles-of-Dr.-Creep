...
**Reproducible on macOS:** `brew install dasm`, then `cd "Creep Sourcecode" && ./build.sh`. It assembles `object.asm` and `creep.asm` to binaries **byte-identical** to `prg/Object.prg` / `prg/Creep.prg` (the 2010 build this source targets — the `*.orig.prg` files are a different, earlier 2008 build). Details in `Creep Sourcecode/BUILD.md`.

The source was written for an older Windows DASM, so `build.sh` assembles from a temp tree and normalises a *copy* for modern DASM (adds `processor 6502`, dedents the `* equ $0800` origin, converts `\`→`/` paths, strips explicit `asl a`-style accumulator operands, and fixes two source typos) — the checked-in `asm/*.asm` and `inc/CC_*.asm` stay byte-for-byte untouched. The seven previously-missing C64 register/constant includes (`vic/sid/cia1/cia2/color/mem/kernel.asm`) are now authored in `inc/`. `PicATitle.prg` is title-screen picture data, not code (no asm). The Windows `asm.bat`/`dis.bat` still invoke DASM at a hardcoded path and won't run here — use `build.sh`.
